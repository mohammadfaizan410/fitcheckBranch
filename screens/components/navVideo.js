...
export default function NavVideo({ navigation, fitcheck }) {
    ...
    const commentIcon = <Font5Icon name="comment-alt"  size={30} color={'#F6F5F4'}/>
    ...
    useEffect(() => {        setIsLiked(likes?.includes(currentusername));
    }, [likes]);
    ...
    listings.forEach(element => {
      ...
      const mappedImages = listingImages?.map(item => {
          return (
            ...
            <Image source={{ uri: uriPrefix + item }} style={{ width: 50, height: 50,  borderColor: 'white',
